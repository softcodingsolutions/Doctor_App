...
function SurveyHealthProblem() {
  const context = useOutletContext();
  const [getHealthProblems, setGetHealthProblems] = useState([]);
  const [selectedCheckboxes, setSelectedCheckboxes] = useState([]);
  const [showCheckboxes, setShowCheckboxes] = useState(false);

  const handleGetHealthProblems = () => {
    axios
      .get(`/api/v2/survey_helth_problems`)
      .then((res) => {
        console.log(res);
        setGetHealthProblems(res.data?.all_survey_helth_problems);
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  const handleSave = async () => {
    const selectedHealth = selectedCheckboxes
      .map((id) => getHealthProblems.find((health) => health.id === Number(id)))
      .filter((health) => health);
    if (selectedHealth.length === 0) {
      return Swal.fire({
        icon: "warning",
        title: "No Health Problems Selected",
        text: "Please select at least one problem to save.",
      });
    }
    console.log("Selected Health Problem: ", selectedHealth);
        ...
        <div className="flex px-4 py-3 h-full flex-col space-y-3">
          <div className="flex gap-5 text-center items-center justify-between">
            ...
            {showCheckboxes && (
              <div className="font-[550] text-lg">
                No. of Health Problem checked: {selectedCheckboxes.length}
              </div>
            )}

            {!showCheckboxes && (
              <div className="font-[550] text-lg flex items-center">
                Checked Health Problem -{" "}
            ...
            <table className="w-full min-w-[460px] z-0">
              <thead className="uppercase ">
                <tr className="bg-[#1F2937] text-white rounded-md">
                  ...
                  <ThComponent name="Health Problem Name" />
                </tr>
              </thead>
              <tbody>
                {getHealthProblems.length === 0 ? (
                  <tr>
                    <th
                  ...
                  getHealthProblems.map((val, index) => {
                    return (
                      <tr
                        className={`${
                          context[2]?.some(
          ...
          {!showCheckboxes && (
            <div className="flex justify-end">
              <NextPageButton
                name="Weight Gain Questions"
                to="../survey-weight-gain-questions"
              />
            </div>
          )}
          {showCheckboxes && (
...
export default SurveyHealthProblem;
